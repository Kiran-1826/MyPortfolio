...
import { useHero } from "../sanity/hooks";
        ...
        <div className="flex flex-col items-center text-center">
          ...
          >
            <div className="flex flex-wrap justify-center gap-2 md:gap-3">
              {(hero.badges ?? []).map((badge, index) => (
                <span
                  key={badge}
                  className={index % 2 === 0
                    ? "px-4 py-1.5 bg-brand-accent/10 border border-brand-accent/30 text-brand-accent rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap"
                    : "px-4 py-1.5 bg-brand-text/5 border border-brand-text/20 text-brand-text rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap"}
                >
                  {badge}
                </span>
              ))}
            ...
            <div className="flex flex-col sm:flex-row gap-6 mt-4 w-full sm:w-auto">
              <motion.a href="#portfolio" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="retro-btn bg-brand-accent text-brand-bg text-center">
                View Works
              </motion.a>
              <motion.a href="#contact" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="retro-btn bg-brand-bg text-brand-text text-center border-brand-text/20">
                Say Hello!
              </motion.a>
            </div>
          </motion.div>
